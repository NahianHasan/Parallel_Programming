...
#define BACKUP_QUEUE_SIZE 20*REDUCER_QUEUE_SIZE
...
void reducer_thread(FILE *fs, int F,int thread_id, int i, int k, int find_flag, char *file_name, char *file_id, char *file_ext, char *file_dir, struct word reducer_word_list[],int *reducer_word_count, unsigned int current_reducer_word_length, unsigned int current_reducer_unique_words, struct word temp_realloc_array, unsigned int reducer_total_word){
	...
	strcpy(file_dir,"../Results/MPI_Reducer_");
	strcpy(file_ext,".txt");
	sprintf(file_id,"%d",F);
				...
				if (hash_number != 10){//removing Data Link Escape
					...
					if (buffer_status[reducer_id]==0){
						...
						if (buffer_word_count[reducer_id] >= (REDUCER_QUEUE_SIZE-NUM_REDUCER)){
							//send the data to corresponding reducer thread
							MPI_send(word_hash[reducer_id], 1, MPI_INT, rank+(i%num_files_per_node), rank+(i%num_files_per_node), MPI_COMM_WORLD)
							//reset the buffer for that thread
							for (i=0;i<REDUCER_QUEUE_SIZE;i++){
								memset(word_hash[reducer_id][i].key,0,sizeof(word_hash[reducer_id][i].key));
							}
						}
					}
					//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
void omp_scheduler(int numP, int rank){
	int initial_file;
	int end_file;
	int num_nodes = (int) (numP/(float)TOTAL_THREADS);
	int num_files_per_node = (int) (NUM_FILES/(float)num_nodes);

	if (rank%TOTAL_THREADS == 0){
		int div_res = (int) (rank/(float) TOTAL_THREADS);
		initial_file = (div_res*num_files_per_node)+1;
		end_file = (div_res+1)*num_files_per_node;
		for (i=initial_file;i<=end_file;i++){
			MPI_send(i, 1, MPI_INT, rank+(i%num_files_per_node), rank+(i%num_files_per_node), MPI_COMM_WORLD)
		}
	}

	int rank_id = rank%TOTAL_THREADS;
	int node_id;
	if (rank_id >= 1 && rank_id <= num_files_per_node){
		int F;
		node_id = (rank/(float)TOTAL_THREADS)*TOTAL_THREADS;
		MPI_Recv(F,1,node_id,node_id,MPI_COMM_WORLD,IGNORE);


		printf("Rank = %d, I_File=%d, E_File = %d\n",rank,initial_file,end_file);
		...
		read_file(F,fp,rank,word_count,file_name,file_id,file_ext,file_dir,p,buff,verbose,delim,reducer_id,hash_number,i,k,tt,temp,word_flag,word_hash,backup_word_hash,lck);
	}
	else if (rank_id > num_files_per_node){
...
int main(int argc, char *argv[]){
	//system("exec rm -r ../Results/MPI_*");//delete the existing results
	...
	start = MPI_Wtime();
	//openmp scheduler
	omp_scheduler(numP,rank);
	MPI_Barrier(MPI_COMM_WORLD);
	end = MPI_Wtime();
	MPI_Finalize();
	if (rank==0){
		printf("Number of Threads = %d\n",TOTAL_THREADS);
		printf("MPI Operation Time: time taken for matrix multiply: %f \n", end-start);
	}
	return 0;
}
