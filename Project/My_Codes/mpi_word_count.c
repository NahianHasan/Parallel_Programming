		...
		#pragma omp single nowait
		{
			...
			for (F=1;F<=TOTAL_THREADS;F++){
				...
				if ((F>=initial_file) & (F<=end_file)){
					#pragma omp task untied
					read_file(F,fp,thread_id,word_count,file_name,file_id,file_ext,file_dir,p,buff,verbose,delim,reducer_id,hash_number,i,k,tt,temp,word_flag,word_hash,backup_word_hash,lck);
				}
			}
		}
	}
