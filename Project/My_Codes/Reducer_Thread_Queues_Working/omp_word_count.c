...
#define REDUCER_WORD_LIST_INCREASE_STEP 1000
	...
	while(1){
		...
		if (buffer_status[F]==1){
			...
			for (i=0;i<REDUCER_QUEUE_SIZE;i++){
				...
				if (DEBUG_MODE==1){
					printf("%d,%d,%d,%d\n",i,current_reducer_unique_words,F,thread_id);
				}
				if (current_reducer_unique_words==current_reducer_word_length){
					...
					/*
					if (temp_realloc_array == NULL){
						printf("\n\nCannor reallocate memory to 'reducer_word_list' in reducer %d, Current length of unique words =  %d\n\n",F,current_reducer_unique_words);
						return;
					}
					else{
						*reducer_word_list = temp_realloc_array;
					}
					*/
					if (DEBUG_MODE==1){
					...
					/*
					if (temp_realloc_array == NULL){
						printf("\n\nCannor reallocate memory to 'reducer_word_count' in reducer %d, Current length of unique words =  %d\n\n",F,current_reducer_unique_words);
						return;
					}
					else{
						*reducer_word_count = temp_realloc_array;
					}
					*/
				...
				if (hash_number != 10){//removing Data Link Escape
					...
					if (buffer_status[reducer_id]==0){
						...
						if (buffer_word_count[reducer_id] >= (REDUCER_QUEUE_SIZE-NUM_REDUCER)){
							buffer_word_count[reducer_id] = 0;
							buffer_status[reducer_id] = 1;
							while(1){
								if (buffer_status[reducer_id]==0){
									if (DEBUG_MODE==1){
										printf("New Batch Reading, Reducer=%d,Thread=%d\n",reducer_id,thread_id);
									}
									break;
								}
							}
...
int main(int argc, char *argv[]){
	system("exec rm -r ../Results/*");//delete the existing results
	...
	*/
	...
	#pragma omp parallel private(F,fp,fs,thread_id,word_count,actual_number,file_name,file_id,file_ext,file_dir,p,buff,verbose,delim,reducer_id,hash_number,i,k,tt,temp,word_flag,find_flag,reducer_word_list,reducer_word_count,current_reducer_word_length,current_reducer_unique_words,temp_realloc_array,reducer_total_word) shared(mapper_stop_flag,reducer_stop_flag,word_hash,buffer_status,buffer_word_count)
	{
		#pragma omp single nowait
		{
			#pragma omp task private(fp,fs,thread_id,word_count,actual_number,file_name,file_id,file_ext,file_dir,p,buff,verbose,delim,reducer_id,hash_number,i,k,tt,temp,word_flag,find_flag,reducer_word_list,reducer_word_count,current_reducer_word_length,current_reducer_unique_words,temp_realloc_array,reducer_total_word) shared(mapper_stop_flag,reducer_stop_flag,word_hash,buffer_status,buffer_word_count)
