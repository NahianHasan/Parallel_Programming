...
void reducer_thread(FILE *fs, int F,int thread_id, int *mapper_stop_flag, int i, int k, int *reducer_occurrences, int occurrence_count, int find_flag, char *file_name, char *file_id, char *file_ext, char *file_dir){
	char *word_end = "\n";
	char *separator = "=";

	strcpy(file_dir,"../Results/Reducer_");
	strcpy(file_ext,".txt");
	sprintf(file_id,"%d",F);
	...
	for (i=0;i<reducer_word_count[F];i++){
		...
		if (strlen(word_hash[F][i].key) > 0) {
			//check whether i is in the count list 'reducer_occurrences'
			if (reducer_occurrences[i] == 0){
				for (k=i;k<reducer_word_count[F];k++){
					if (!strcmp(word_hash[F][k].key,word_hash[F][i].key)){
						occurrence_count++;
						reducer_occurrences[k] = 1;
						find_flag=1;
					}
				}
			}
			if (find_flag==1){
				fputs(word_hash[F][i].key, fs);
				fputs(separator,fs);
				fprintf(fs, "%d", occurrence_count);
				fputs(word_end,fs);
			}
		}
	}
	fclose(fs);
	printf("R=%d,--%d\n",F,reducer_word_count[F]);

	printf("Exiting Reducer = %d, Thread ID = %d\n",F,thread_id);
	return;
}
...
void read_file(int F, FILE *fp, int thread_id,unsigned int word_count, int actual_number, char *file_name, char *file_id, char *file_ext, char *file_dir, char *p, char *buff, int verbose, char *delim,int reducer_id,int hash_number, int i, int tt, char *temp,int word_flag, int *mapper_stop_flag, struct word word_hash[][REDUCER_QUEUE_SIZE],int *reducer_word_count){
	...
	strcpy(file_name,strcat(file_id,file_ext));
	strcpy(file_name,strcat(file_dir,file_name));
	printf("Opening file, Thread = %d, File = %s\n",thread_id,file_name);
	...
	while((fgets(buff, BUFFER_LENGTH, (FILE*)fp)) != NULL){
		p = (char*)malloc(BUFFER_LENGTH * sizeof(char));
...
int main(int argc, char *argv[]){
	int reducer_occurrences[REDUCER_QUEUE_SIZE];//Keep track of indices which have been counted in the reducer queues for occurrence.
	...
	*/
	...
	#pragma omp parallel private(F,fp,fs,thread_id,word_count,actual_number,file_name,file_id,file_ext,file_dir,p,buff,verbose,delim,reducer_id,hash_number,i,k,tt,temp,word_flag,reducer_occurrences,occurrence_count,find_flag) shared(mapper_stop_flag,reducer_stop_flag,word_hash,reducer_word_count)
	{
		#pragma omp single nowait
		{
			#pragma omp task private(fp,fs,thread_id,word_count,actual_number,file_name,file_id,file_ext,file_dir,p,buff,verbose,delim,reducer_id,hash_number,i,k,tt,temp,word_flag,reducer_occurrences,occurrence_count,find_flag) shared(mapper_stop_flag,reducer_stop_flag,word_hash,reducer_word_count)
			for (F=1;F<=TOTAL_THREADS;F++){
				//Spawn the reducer threads first
				if (F<=NUM_REDUCER){
					#pragma omp task untied
					reducer_thread(fs,F,thread_id,mapper_stop_flag,i,k,reducer_occurrences,occurrence_count,find_flag,file_name,file_id,file_ext,file_dir);
