...
void printArray(double* a, int rows, int cols, int sparseness) {
   ...
   fflush(stdout);
   for (i=0; i<rows; i+=sparseness) {
      for (j=0; j<cols; j+=sparseness) {
...
MPI_Datatype makeType( ) {
		...
		MPI_Type_vector(LOCAL_ARRAY_ROWS,LOCAL_ARRAY_COLS,LOCAL_ARRAY_COLS,MPI_DOUBLE,&block);
		MPI_Type_commit(&block);
		MPI_Type_create_resized(block,0,LOCAL_ARRAY_ROWS*LOCAL_ARRAY_COLS*sizeof(double),&blocktype);
...
int main (int argc, char *argv[]) {
   ...
   for (iter=0; iter<SQRT_P; iter++) {
      for (i=0; i<LOCAL_ARRAY_ROWS; i++) {
         for (j=0; j<LOCAL_ARRAY_COLS; j++) {
            int comp = 0.;
            for (k=0; k<LOCAL_ARRAY_ROWS; k++) {
